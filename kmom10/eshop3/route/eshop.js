...
const terminal = require("../src/terminal-eshop.js");
...
router.get("/about", async (req, res) => {
    ...
    data.about = await eshop.showAbout();
...
// SHOW PRODUCTS SORTES CATEGORIES
router.get("/category-prod/:id", async (req, res) => {
    let catid = req.params.id;
    let data = {
        title: "Products sorted category | The Shop",
    };

    data.cat = await eshop.showCategory();
    data.prod = await eshop.showCatProd(catid);

    res.render("eshop/category-prod", data);
});
...
// LOG
router.get("/log/:num", async (req, res) => {
    ...
    data.log = await terminal.showLog(num);

    res.render(`eshop/log`, data);
});

// LOG SORTED
router.post("/log-sorted", urlencodedParser, async (req, res) => {
    let num = req.body.num;

    res.redirect(`/eshop/log/${num}`);
});

// LOG SORTED WITH SEARCH STRING
router.get("/log-search/:s", async (req, res) => {
    let search = req.params.s;

    let data = {
        title: "Log - search result| The Shop",
        search: search
    };

    data.log = await terminal.showLogSearch(search);
    res.render(`eshop/log-search`, data);
});

router.post("/log-search-post", urlencodedParser, async (req, res) => {
    let search = req.body.search;

    res.redirect(`/eshop/log-search/${search}`);
});
